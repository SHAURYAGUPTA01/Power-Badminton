class Game {
    ...
    play() {
        ...
        if (allPlayers !== undefined) {
            ...
            for (var plr in allPlayers) {
                ...
                text("instructions:", 200, 10, 30, 30)
                text("press S to start", 200, 25, 100, 30)
            }
            push()
            ...
            pop()
            if (keyIsDown(83) && shuttle.body.position.x > 0) {
                Matter.Body.setStatic(shuttle.body, false)
                Matter.Body.applyForce(shuttle.body, { x: 0, y: 0 }, { x: 5, y: 100 })
            }
        }
        shuttle.display();
    ...
    handleBadmintonControls() {
        console.log(angle1)
        if (keyIsDown(85) && badminton1.angle < 115) {
            console.log("test")
